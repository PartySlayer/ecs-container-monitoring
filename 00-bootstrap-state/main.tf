...
data "aws_iam_policy_document" "github_trust_policy" {
  statement {
    ...
    condition {
      ...
      values   = ["repo:PartySlayer/serverless-container-monitoring"]
    }
  }
}
...
resource "aws_iam_role_policy" "terraform_permissions" {
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      {
        Sid    = "TerraformResourceDeployment"
        Effect = "Allow"
        Action = [
          # EC2 Core, best practice wildcard
          "ec2:*",
          ...
          "iam:DeleteRole",
          "iam:GetRole",
          "iam:PassRole", # permette a Terraform di assegnare un ruolo a ECS o EC2
          "iam:TagRole",
          "iam:UntagRole",
